...
-- get sound of sura
local function read_sura(channel_id, target)
 local readq = http.request(umbrella.."Sura"..target..".mp3")
 local url = umbrella.."Sura"..target..".mp3"
 local file = download_to_file(url)
 local cb_extra = {file_path=file}
 return send_document("channel#id"..channel_id, file, rmtmp_cb, cb_extra)
end
...
local function run(msg, matches)
 ...
 if matches[1] == "read" then
  ...
  return read_sura(channel_id, target)
  
 elseif matches[1] == "sura" then
  local target = matches[2]
  return view_sura(channel_id, target)
